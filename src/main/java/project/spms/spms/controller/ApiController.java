...
public class ApiController {
    ...
    @Autowired
    private CompanyRepository companyRepo;  // Bug Fix #8: company registration
    ...
    public ResponseEntity<ApiResponse> getNotifications(@RequestParam Integer userId) {
        ...
        long unreadCount = notifs.stream().filter(n -> !Boolean.TRUE.equals(n.getIsRead())).count();
        Map<String, Object> result = new HashMap<>();
    ...
    // ════════════════════════════════════════════════════════════
    // MESSAGES  (Bug Fix #5: attachments | Bug Fix #6: isolation)
    // ════════════════════════════════════════════════════════════

    /**
     * POST /api/messages — send a plain text message (no attachment).
     * Bug Fix #6: Only messages belonging to the authenticated user are returned.
     */
    ...
    /**
     * POST /api/messages/with-attachment — send a message with a file attachment.
     * Bug Fix #5: Integrated file storage for message attachments.
     */
    @PostMapping("/messages/with-attachment")
    @Transactional
    public ResponseEntity<ApiResponse> sendMessageWithAttachment(
        ...
        try {
            ...
            if (file != null && !file.isEmpty()) {
                String filepath = fileStorage.store(file, 0);
                m.setAttachmentFilename(file.getOriginalFilename());
                m.setAttachmentFilepath(filepath);
                m.setAttachmentFilesize(fileStorage.humanSize(file.getSize()));
            }
            Message saved = messageRepo.save(m);
            String senderName = userRepo.findById(fromId).map(User::getName).orElse("Someone");
            notifService.sendInfo(toId, "Message from " + senderName, body, "overview");
            return ok(ApiResponse.ok("Message sent", saved));
        } catch (Exception e) {
            return ok(ApiResponse.error("Send failed: " + e.getMessage()));
        }
    }

    /**
     * GET /api/messages?userId=X — inbox (messages received by userId).
     * Bug Fix #6: Only returns messages where to_id = userId.
     */
    @GetMapping("/messages")
    ...
    /**
     * GET /api/messages/conversation?userId=X&partnerId=Y
     * Bug Fix #6: Returns the full bilateral conversation. Marks received messages as read.
     */
    ...
    public ResponseEntity<ApiResponse> getConversation(
        ...
        try {
            messageRepo.markAllReadInConversation(userId, partnerId);
        } catch (Exception ignored) {}
        List<Message> msgs = messageRepo.findConversation(userId, partnerId);
        return ok(ApiResponse.ok(msgs));
    }
    ...
    @Transactional
    public ResponseEntity<ApiResponse> markMessageRead(@PathVariable Integer id) {
        try {
    ...
    /**
     * GET /api/messages/attachment/{id}/download — download a message attachment.
     * Bug Fix #5: Attachment download endpoint.
     */
    @GetMapping("/messages/attachment/{id}/download")
    public ResponseEntity<Resource> downloadMessageAttachment(@PathVariable Integer id) {
        try {
            Message m = messageRepo.findById(id)
                    .orElseThrow(() -> new RuntimeException("Message not found"));
            if (m.getAttachmentFilepath() == null || m.getAttachmentFilepath().isBlank())
                return ResponseEntity.notFound().build();
            Resource resource = fileStorage.load(m.getAttachmentFilepath());
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION,
                            "attachment; filename=\"" + m.getAttachmentFilename() + "\"")
     ...
     * Bug Fix #7: totalUsers is now always live from DB so the homepage counter
     * reflects actual user count after add/remove operations.
     */
    ...
    public ResponseEntity<ApiResponse> systemHealth() {
        ...
        // Bug Fix #7: always query live count so homepage counter stays accurate
        long liveUserCount = userRepo.count();
        health.put("totalUsers", liveUserCount);
        health.put("activeUsers", userRepo.findByStatus(User.Status.active).size());
        health.put("totalProjects", projRepo.count());
        health.put("totalFiles", fileRepo.count());
        health.put("totalCompanies", companyRepo.count());
    ...
    // ════════════════════════════════════════════════════════════
    // COMPANY REGISTRATION  (Bug Fix #8)
    // ════════════════════════════════════════════════════════════

    /**
     * POST /api/register — public endpoint to register a new company.
     * Creates the company record and one admin user automatically.
     * Bug Fix #8: SPMS now supports company-based registration, not individual users.
     */
    @PostMapping("/register")
    @Transactional
    public ResponseEntity<ApiResponse> registerCompany(@RequestBody Map<String, Object> body) {
        try {
            String companyName  = (String) body.get("companyName");
            String companyEmail = (String) body.get("companyEmail");
            String companyPhone = (String) body.getOrDefault("companyPhone", "");
            String industry     = (String) body.getOrDefault("industry", "");
            String address      = (String) body.getOrDefault("address", "");
            String regNumber    = (String) body.getOrDefault("registrationNumber", "");
            String website      = (String) body.getOrDefault("website", "");

            // Admin account details for the company
            String adminName     = (String) body.get("adminName");
            ...
            if (companyName == null || companyEmail == null)
                return ok(ApiResponse.error("Company name and email are required"));
            if (adminName == null || adminUsername == null || adminPassword == null)
                return ok(ApiResponse.error("Admin name, username and password are required"));
            if (companyRepo.existsByCompanyEmail(companyEmail))
                return ok(ApiResponse.error("A company with that email is already registered"));
            if (companyRepo.existsByCompanyName(companyName))
                return ok(ApiResponse.error("A company with that name is already registered"));
            ...
            admin.setEmail(adminEmail != null ? adminEmail : companyEmail);
            admin.setUsername(adminUsername);
            admin.setRole(User.Role.admin);
            admin.setCompanyId(savedCompany.getId());
            admin.setDepartmentName("Administration");
            User savedAdmin = userService.create(admin, adminPassword, null, "System Registration");

            Map<String, Object> result = new HashMap<>();
            result.put("company", savedCompany);
            result.put("admin", savedAdmin);
            result.put("message", "Company registered successfully. You can now log in with your admin account.");

            audit.log(savedAdmin.getId(), savedAdmin.getName(),
                    "Company registered: " + companyName, AuditLog.AuditType.create);

            return ok(ApiResponse.ok("Registration successful", result));
        } catch (Exception e) {
            return ok(ApiResponse.error("Registration failed: " + e.getMessage()));
        }
    }

    /** GET /api/companies — list all companies (admin only) */
    @GetMapping("/companies")
    public ResponseEntity<ApiResponse> listCompanies() {
        return ok(ApiResponse.ok(companyRepo.findAll()));
    }
    ...
    public ResponseEntity<ApiResponse> getCompany(@PathVariable Integer id) {
        ...
        if (opt.isEmpty()) return ok(ApiResponse.error("Company not found"));
        return ok(ApiResponse.ok(opt.get()));
    }
