...
async function initDashboard(role, user) {
    ...
    // Bug Fix #7: Update landing page counters from live system health data
    api('GET', '/system/health').then(h => {
        const data = h?.data || {};
        const liveCount = data.totalUsers || 0;
        if (liveCount > 0) {
            const elUsers = document.getElementById('counterUsers');
            if (elUsers) {
                elUsers.setAttribute('data-count', liveCount);
                elUsers.textContent = liveCount;
            }
            // Also update the stat-banner-value span that uses data-count
            document.querySelectorAll('.stat-banner-value[data-suffix="+"]').forEach((el, i) => {
                if (i === 0) { // first one is users
                    el.setAttribute('data-count', liveCount);
...
async function logout() {
    ...
    // Bug Fix #6: clear message caches so next user cannot see previous user's messages
    ...
    destroyAllCharts();
    // Bug Fix #4: stop background polling on logout
    if (window._adminActivityPollInterval) {
        clearInterval(window._adminActivityPollInterval);
        window._adminActivityPollInterval = null;
    }
...
async function initHRDashboard(user) {
    ...
    // Bug Fix #4: poll audit log and stats every 30 seconds so activity is
    // visible instantly without needing a server restart or manual page refresh.
    if (window._adminActivityPollInterval) {
        clearInterval(window._adminActivityPollInterval);
    }
    window._adminActivityPollInterval = setInterval(async () => {
        // Only poll if admin is still logged in
        if (!currentUser || currentUser.role !== 'admin') {
            clearInterval(window._adminActivityPollInterval);
            return;
        }
        try {
            // Refresh user and project lists silently
            const [uRes, pRes] = await Promise.all([
                api('GET', '/users').catch(() => null),
                api('GET', '/projects').catch(() => null)
            ]);
            if (uRes?.data)  allUsers    = uRes.data;
            if (pRes?.data)  allProjects = pRes.data;
            renderAdminStats();
            // Refresh audit log only if the audit view is currently visible
            const auditView = document.getElementById('admin-audit');
            if (auditView && !auditView.classList.contains('hidden')) {
                renderAuditLog();
            }
        } catch { /* silent fail — do not spam toasts */ }
    }, 30000);
}
...
async function handleAddUser(event) {
    ...
    try {
        ...
        renderUserManagement();
        renderAdminStats();
        // Bug Fix #7: refresh homepage live user counter after creation
        refreshUserCounter();
...
async function deleteUser(userId) {
    try {
        ...
        renderUserManagement();
        renderAdminStats();
        // Bug Fix #7: refresh homepage live user counter after deletion
        refreshUserCounter();
...
function exportProjectReport(projectId) {
    // Bug Fix #1: Generate a real PDF instead of a CSV blob.
    ...
    try {
        ...
        doc.setFontSize(11);
        doc.text('SPMS — Project Report', margin, 9.5);
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text(`${p.name} (${p.code})`, margin, y);
        y += 8;
        ...
        doc.text(`Generated: ${new Date().toLocaleString()}   |   Status: ${(p.status||'').toUpperCase()}   |   Progress: ${p.progress||0}%`, margin, y);
        y += 2;
        doc.setTextColor(0,0,0);
        ...
        const details = [
            ...
            ['Deadline',    p.deadline     || '—'],
            ['Department',  p.departmentName || p.department_name || '—'],
        ...
        const pct = Math.min(100, parseInt(p.progress)||0);
        if (pct > 0) {
            doc.setFillColor(99, 102, 241);
            doc.roundedRect(margin, y, (pageW - margin*2) * pct / 100, 6, 2, 2, 'F');
        }
        doc.setFontSize(8);
        doc.setTextColor(255,255,255);
        if (pct > 10) doc.text(`${pct}%`, margin + 3, y + 4.2);
        doc.setTextColor(0,0,0);
        y += 12;
        ...
        if (team.length > 0) {
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(11);
            doc.text(`Team Members (${team.length})`, margin, y);
            y += 6;

            // Table header
            const cols = [margin, margin+60, margin+100, margin+130];
            doc.setFillColor(240, 240, 250);
            doc.rect(margin, y - 4, pageW - margin*2, 7, 'F');
            doc.setFontSize(9);
            doc.setFont('helvetica', 'bold');
            ['Name', 'Role in Project', 'Commits', 'Hours'].forEach((h, i) => doc.text(h, cols[i], y));
            y += 5;
            doc.setFont('helvetica', 'normal');
            team.forEach((m, idx) => {
                ...
                doc.text(String(m.name||'—').substring(0,28), cols[0], y);
                doc.text(String(m.roleInProject||m.role_in_project||'Member').substring(0,20), cols[1], y);
                doc.text(String(m.commits||0), cols[2], y);
                doc.text(String(m.hoursContributed||m.hours_contributed||0), cols[3], y);
                y += 6;
            });
        }
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            doc.text(`SPMS Project Report — ${p.code} — Page ${i} of ${totalPages}`, margin, 292);
            doc.setTextColor(0, 0, 0);
        }

        doc.save(`${p.code}_report.pdf`);
        showToast(`${p.code} PDF exported ✓`);
    } catch (err) {
        console.error('PDF export error:', err);
        // Fallback to CSV if jsPDF is somehow not loaded
        const team2 = p.team || [];
        const csv = `Project Report\nCode,${p.code}\nName,${p.name}\nStatus,${p.status}\nProgress,${p.progress}%\nDeadline,${p.deadline}\n`;
        downloadCSV(csv, `${p.code}_report.csv`);
        showToast(`Exported as CSV (PDF library unavailable)`, 'error');
    }
}

function exportEmployeeReport(empId) {
    // Bug Fix #1: generate a proper PDF instead of a CSV blob
    const e = allUsers.find(u=>u.id===empId);
    if (!e) return;
    const skills = parseSkills(e.skills);
    const empId2 = e.employeeId || e.employee_id || '—';
    const dept   = e.departmentName || e.department_name || '—';
    const perf   = e.performanceScore || e.performance_score || 0;
    const wl     = e.workload || 0;
    try {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF({ unit: 'mm', format: 'a4' });
        const pageW = doc.internal.pageSize.getWidth();
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(11);
        doc.text('SPMS — Employee Report', margin, 9.5);
        doc.setTextColor(0,0,0);
        y = 24;

        // Avatar circle
        doc.setFillColor(99, 102, 241);
        doc.circle(pageW - margin - 12, y + 4, 10, 'F');
        doc.setTextColor(255,255,255);
        doc.setFontSize(13);
        doc.setFont('helvetica', 'bold');
        const initials = e.avatarInitials || e.name.substring(0,2).toUpperCase();
        doc.text(initials, pageW - margin - 16, y + 6);
        doc.setTextColor(0,0,0);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(120,120,120);
        doc.text(`${empId2}   |   ${(e.role||'employee').toUpperCase()}   |   ${dept}`, margin, y);
        doc.setTextColor(0,0,0);
        y += 3;
        doc.setDrawColor(99, 102, 241);
        doc.setLineWidth(0.5);
        ...
        const rows = [
            ...
            ['Join Date',        e.joinDate || e.join_date || '—'],
            ['Hours / Week',     `${e.hoursPerWeek || e.hours_per_week || 40}h`],
            ['Workload',         `${wl}%`],
        ...
        doc.setFillColor(230, 230, 230);
        doc.roundedRect(margin, y, pageW - margin*2, 6, 2, 2, 'F');
        const perfPct = Math.min(100, perf);
        const barColour = perfPct >= 80 ? [34,197,94] : perfPct >= 60 ? [234,179,8] : [239,68,68];
        doc.setFillColor(...barColour);
        doc.roundedRect(margin, y, (pageW - margin*2) * perfPct / 100, 6, 2, 2, 'F');
        doc.setFontSize(8);
        doc.setTextColor(255,255,255);
        if (perfPct > 10) doc.text(`${perfPct}%`, margin + 3, y + 4.2);
        doc.setTextColor(0,0,0);
        y += 12;
        ...
        if (skills && skills.length) {
            ...
            for (let i = 0; i < skills.length; i += perRow) {
                ...
                row.forEach((sk, j) => {
                    const sx = margin + j * 44;
                    doc.setFillColor(237, 233, 254);
                    doc.roundedRect(sx, y - 3, 40, 6, 2, 2, 'F');
                    doc.setTextColor(79, 70, 229);
                    doc.text(sk.substring(0,14), sx + 2, y + 0.5);
                    doc.setTextColor(0,0,0);
                });
                y += 8;
            }
        ...
        doc.setFontSize(7);
        doc.setTextColor(150,150,150);
        doc.text(`SPMS Employee Report — ${empId2} — ${new Date().toLocaleString()}`, margin, 292);

        doc.save(`${empId2}_report.pdf`);
        showToast(`${e.name} PDF exported ✓`);
    } catch (err) {
        console.error('PDF export error:', err);
        const csv = `Employee Report\nName,${e.name}\nID,${empId2}\nDept,${dept}\nPerformance,${perf}%\nWorkload,${wl}%\n\nSkills\n${skills.join('\n')}`;
        downloadCSV(csv, `${empId2}_report.csv`);
        showToast('Exported as CSV (PDF library unavailable)', 'error');
    }
}

function exportData(type) {
    // Bug Fix #1: All exports now generate properly formatted PDFs.
    // jsPDF is loaded from CDN in index.html.
    const map = {
        'manager-report':        () => ({ title:'Manager Project Report',      headers:['Code','Name','Status','Priority','Progress','Deadline','Team Size'], rows: allProjects.map(p=>[p.code,p.name,p.status||'—',p.priority||'—',(p.progress||0)+'%',p.deadline||'—',(p.team||[]).length]), file:'manager_report' }),
        'hr-projects':           () => ({ title:'HR Projects Overview',         headers:['Code','Name','Status','Urgency','Deadline'],                         rows: allProjects.map(p=>[p.code,p.name,p.status||'—',p.urgency||'—',p.deadline||'—']),                                    file:'hr_projects' }),
        'admin-report':          () => ({ title:'Admin User Report',            headers:['ID','Name','Role','Department','Status','Performance'],              rows: allUsers.map(u=>[(u.employeeId||u.employee_id||'—'),u.name,u.role,(u.departmentName||u.department_name||'—'),u.status||'—',(u.performanceScore||u.performance_score||0)+'%']), file:'admin_report' }),
        'employee-tracking-csv': () => ({ title:'Employee Tracking Report',     headers:['ID','Name','Department','Performance','Workload'],                   rows: allUsers.filter(u=>u.role==='employee').map(e=>[(e.employeeId||e.employee_id||'—'),e.name,(e.departmentName||e.department_name||'—'),(e.performanceScore||e.performance_score||0)+'%',(e.workload||0)+'%']), file:'employee_tracking' }),
        'workload-report':       () => ({ title:'Employee Workload Report',     headers:['ID','Name','Department','Workload','Hours/Week'],                    rows: allUsers.filter(u=>u.role==='employee').map(e=>[(e.employeeId||e.employee_id||'—'),e.name,(e.departmentName||e.department_name||'—'),(e.workload||0)+'%',(e.hoursPerWeek||e.hours_per_week||40)+'h']), file:'workload_report' })
    };
    const gen = map[type];
    const spec = gen ? gen() : { title: type, headers: ['Export Type','Date','Generated By'], rows: [[type, new Date().toLocaleDateString(), currentUser?.name||'User']], file: type };
    _generateTablePDF(spec.title, spec.headers, spec.rows, spec.file + '.pdf');
}

function _generateTablePDF(title, headers, rows, filename) {
    try {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF({ orientation: rows[0] && rows[0].length > 5 ? 'landscape' : 'portrait', unit: 'mm', format: 'a4' });
        const pageW = doc.internal.pageSize.getWidth();
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(10);
        doc.text('SPMS — ' + title, margin, 9.5);
        doc.setTextColor(0,0,0);
        y = 22;
        ...
        doc.text(`Generated: ${new Date().toLocaleString()}   |   By: ${currentUser?.name||'User'}   |   Total Records: ${rows.length}`, margin, y);
        doc.setTextColor(0,0,0);
        y += 3;
        doc.setDrawColor(99,102,241);
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(8);
        headers.forEach((h, i) => doc.text(String(h).substring(0,16), margin + i*colW + 1, y));
        doc.setTextColor(0,0,0);
        y += 5;
        ...
        rows.forEach((row, ri) => {
            ...
            row.forEach((cell, ci) => {
                doc.text(String(cell==null?'—':cell).substring(0,18), margin + ci*colW + 1, y);
            });
            y += 6;
        });
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            doc.setFontSize(7);
            doc.setTextColor(150,150,150);
            doc.text(`SPMS — ${title} — Page ${i} of ${totalPages}`, margin, pageH - 5);
            doc.setTextColor(0,0,0);
        }
...
function initLandingAnimations() {
    ...
    // ── Bug Fix #3: Smooth typewriter effect replaces the jarring scramble ──
    // The old scramble replaced every character with random ASCII chars simultaneously,
    // making the text unreadable. The new typewriter builds the text left-to-right,
    // character by character, with a blinking cursor — clean and professional.
    const scrambleEl = document.querySelector('.scramble-text');
    if (scrambleEl) {
        const original = scrambleEl.textContent.trim();
        scrambleEl.textContent = '';
        scrambleEl.style.borderRight = '2px solid var(--primary, #6366f1)';
        scrambleEl.style.paddingRight = '2px';
        scrambleEl.style.whiteSpace = 'nowrap';
        scrambleEl.style.overflow = 'hidden';

        let charIndex = 0;
        const typeDelay = 45; // ms per character — feels natural, not rushed

        // Blinking cursor animation via CSS class
        if (!document.getElementById('typewriterCursorStyle')) {
            const styleTag = document.createElement('style');
            styleTag.id = 'typewriterCursorStyle';
            styleTag.textContent = `
                @keyframes typewriterBlink {
                    0%, 100% { border-color: var(--primary, #6366f1); }
                    50%       { border-color: transparent; }
                }
                .scramble-text { animation: typewriterBlink 0.8s step-end infinite; }
            `;
        ...
        setTimeout(() => {
            const typeTimer = setInterval(() => {
                if (charIndex < original.length) {
                    scrambleEl.textContent += original.charAt(charIndex);
                    charIndex++;
                } else {
                    clearInterval(typeTimer);
                    // Remove blinking cursor after 2.5 s so it does not distract
                    setTimeout(() => {
                        scrambleEl.style.borderRight = 'none';
                        scrambleEl.style.paddingRight = '0';
                        scrambleEl.style.animation = 'none';
                    }, 2500);
                }
            }, typeDelay);
        }, 600); // small initial delay so page has settled
    }
...
// ── Bug Fix #3: Scroll-triggered section title reveal (replaces jarring scramble) ──
// Instead of replacing every letter with random characters (unreadable), we now
// use a smooth typewriter effect for section titles when they scroll into view.
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            const timer = setInterval(() => {
                if (charIndex < original.length) {
                    el.textContent += original.charAt(charIndex);
                    charIndex++;
                } else {
                    clearInterval(timer);
                }
...
// Bug Fix #7: helper to refresh the live user counter on the homepage
function refreshUserCounter() {
    api('GET', '/system/health').then(h => {
        const liveCount = h?.data?.totalUsers || 0;
        if (liveCount > 0) {
            const elUsers = document.getElementById('counterUsers');
            if (elUsers) {
                elUsers.setAttribute('data-count', liveCount);
...
async function changePassword(userId) {
    ...
    if (success) {
        ...
        // Bug Fix #2: Force the user to re-authenticate with the new password.
        // The backend now saves the new BCrypt hash immediately so the old
        // session is stale. We must log out so the next login uses the new hash.
        showToast('Password updated ✓ — Please log in again with your new password', 'success');
        setTimeout(() => {
            logout();
        }, 2000);
    } else {
        showToast(`Password change failed. Please try again or contact your administrator.`, 'error');
    }
}
...
async function openThread(partnerId, partnerName) {
    ...
    try {
        ...
        // Bug Fix #6: use the dedicated conversation endpoint which:
        //   a) returns BOTH sent and received messages for this pair only
        //   b) marks all received messages in this conversation as read server-side
        //   c) never leaks other users' messages because userId is enforced server-side
        let all = [];
        try {
            const r = await api('GET', `/messages/conversation?userId=${myId}&partnerId=${pid}`);
            all = (r?.data || []).sort((a, b) =>
                new Date(a.created_at || a.createdAt || 0) - new Date(b.created_at || b.createdAt || 0)
            );
        } catch {
            // Fallback: merge inbox + local sent cache if new endpoint not yet deployed
            let received = [];
            try {
                const r2 = await api('GET', `/messages?userId=${myId}`);
                received = r2?.data || [];
            } catch {}
            const allMsgs = [...received, ..._sentMessages];
            const seen = new Set();
        ...
        if (!all.length) {
            ...
            // Bug Fix #5: Render attachment link in thread if message has an attachment
            threadArea.innerHTML = all.map(m => {
                ...
                const attachFilename = m.attachment_filename || m.attachmentFilename;
                const attachSize     = m.attachment_filesize  || m.attachmentFilesize || '';
                const API_BASE = window.API_BASE || (window.location.hostname === 'localhost'
                    ? 'http://localhost:9090/api'
                    : 'https://spms-backend.onrender.com/api');
                const attachHtml = attachFilename
                    ? `<div style="margin-top:0.5rem;padding:0.4rem 0.6rem;background:${isMine?'rgba(255,255,255,0.15)':'var(--bg-primary)'};border-radius:6px;display:flex;align-items:center;gap:0.4rem;font-size:0.78rem">
                           <span>📎</span>
                           <a href="${API_BASE}/messages/attachment/${m.id}/download"
                              style="color:${isMine?'#fff':'var(--primary)'};text-decoration:underline;word-break:break-all"
                              target="_blank" rel="noopener noreferrer">${attachFilename}</a>
                           ${attachSize ? `<span style="opacity:0.65;margin-left:auto;white-space:nowrap">${attachSize}</span>` : ''}
                       </div>`
                    : '';
...
// ════════════════════════════════════════════════════════════════════
// Bug Fix #8 — Company Registration Functions
// ════════════════════════════════════════════════════════════════════
...
function openRegisterModal() {
    ...
    ['regCompanyName','regCompanyEmail','regIndustry','regPhone','regRegNumber',
     'regWebsite','regAddress','regAdminName','regAdminEmail','regAdminUsername',
     'regAdminPassword','regAdminPasswordConfirm'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = '';
    });
    ...
    if (btn) { btn.disabled = false; btn.textContent = 'Register Company 🚀'; }
    modal.classList.add('active');
}
...
async function submitRegistration() {
    ...
    const companyName     = get('regCompanyName');
    const companyEmail    = get('regCompanyEmail');
    const industry        = get('regIndustry');
    const companyPhone    = get('regPhone');
    const registrationNumber = get('regRegNumber');
    const website         = get('regWebsite');
    const address         = get('regAddress');
    const adminName       = get('regAdminName');
    const adminEmail      = get('regAdminEmail');
    const adminUsername   = get('regAdminUsername');
    const adminPassword   = get('regAdminPassword');
    const adminConfirm    = get('regAdminPasswordConfirm');
    ...
    if (!companyEmail)  return showErr('Company email is required.');
    if (!adminName)     return showErr('Admin full name is required.');
    if (!adminUsername) return showErr('Admin username is required.');
    if (!adminPassword) return showErr('Admin password is required.');
    if (adminPassword.length < 6) return showErr('Password must be at least 6 characters.');
    if (adminPassword !== adminConfirm) return showErr('Passwords do not match.');
    ...
    try {
        const res = await api('POST', '/register', {
            ...
            address, registrationNumber, website,
            adminName, adminEmail, adminUsername, adminPassword
        });

        if (res?.status === 'error' || !res?.data) {
            showErr(res?.message || 'Registration failed. Please try again.');
            if (btn) { btn.disabled = false; btn.textContent = 'Register Company 🚀'; }
            return;
        }
        ...
        // Success
        if (okEl) {
            okEl.textContent = `✅ Company "${companyName}" registered successfully! You can now log in with username "${adminUsername}" and role "admin".`;
            okEl.style.display = 'block';
        }
        if (errEl) errEl.style.display = 'none';
        if (btn) { btn.textContent = '✅ Registered!'; }

        // After 4 seconds, close and open login modal
        setTimeout(() => {
            closeModal('registerModal');
            openLoginModal();
            showToast(`Welcome, ${adminName}! Please log in with your new admin account.`, 'success');
        }, 4000);

    } catch (e) {
        showErr('Registration failed: ' + (e.message || 'Unknown error'));
        if (btn) { btn.disabled = false; btn.textContent = 'Register Company 🚀'; }
    }
}
...
// ════════════════════════════════════════════════════════════════════
// Bug Fix #5 — Message Attachment Preview
// ════════════════════════════════════════════════════════════════════
function previewMsgAttachment(input) {
    const previewEl = document.getElementById('msgAttachPreview');
    if (!previewEl) return;
    ...
    const iconMap = { pdf:'📄', doc:'📝', docx:'📝', xls:'📊', xlsx:'📊', ppt:'📋', pptx:'📋', png:'🖼️', jpg:'🖼️', jpeg:'🖼️', gif:'🖼️', zip:'🗜️', txt:'📃', csv:'📊' };
    const icon = iconMap[ext] || '📎';
    previewEl.innerHTML = `<div style="display:flex;align-items:center;gap:0.4rem;background:var(--bg-secondary);padding:0.4rem 0.6rem;border-radius:6px;border:1px solid var(--border-color)">
        <span>${icon}</span>
        <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${file.name}</span>
        <span style="color:var(--text-secondary);font-size:0.75rem;white-space:nowrap">${displaySize}</span>
        <button type="button" onclick="clearMsgAttachment()" style="background:none;border:none;cursor:pointer;color:var(--text-secondary);font-size:1rem;line-height:1;padding:0" title="Remove attachment">✕</button>
    </div>`;
}
window.previewMsgAttachment = previewMsgAttachment;

function clearMsgAttachment() {
    const input = document.getElementById('msgAttachFile');
    const preview = document.getElementById('msgAttachPreview');
    if (input)   input.value = '';
    if (preview) preview.innerHTML = '';
}
window.clearMsgAttachment = clearMsgAttachment;
...
async function sendComposedMessage() {
    ...
    // Bug Fix #5: check for attached file
    const fileInput = document.getElementById('msgAttachFile');
    ...
    try {
        let savedMsg;
        if (hasFile) {
            // Bug Fix #5: send as multipart/form-data to include file attachment
            const API_BASE = window.API_BASE || (window.location.hostname === 'localhost'
                ? 'http://localhost:9090/api'
                : 'https://spms-backend.onrender.com/api');
            const fd = new FormData();
            fd.append('fromId',  String(currentUser.id));
            fd.append('toId',    String(toId));
            fd.append('subject', subj);
            fd.append('body',    body);
            fd.append('file',    fileInput.files[0]);
            const resp = await fetch(`${API_BASE}/messages/with-attachment`, {
                method: 'POST',
                body: fd
            });
            savedMsg = await resp.json();
            if (savedMsg?.status === 'error') throw new Error(savedMsg.message || 'Upload failed');
        } else {
            savedMsg = await _postMessage(currentUser.id, toId, subj, body);
        }
        ...
        if (fileInput) { fileInput.value = ''; document.getElementById('msgAttachPreview') && (document.getElementById('msgAttachPreview').innerHTML = ''); }
