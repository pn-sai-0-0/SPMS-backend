-- ============================================================
--  SPMS v2 — Complete MySQL Schema + Seed Data (TRIGGER-FREE)
--  Safe to run directly in DBeaver / any MySQL client
--  27 Users: 1 Admin | 2 HR | 4 Managers | 20 Employees
...
DROP TABLE IF EXISTS file_deletion_log;
DROP TABLE IF EXISTS manager_daily_log;
DROP TABLE IF EXISTS employee_commits;
...
-- ============================================================
-- COMPANIES  (Bug Fix #8: company-based registration)
-- ============================================================
...
CREATE TABLE messages (
    ...
    body                 TEXT NOT NULL,
    is_read              TINYINT(1) DEFAULT 0,
    -- Bug Fix #5: file attachment support
    attachment_filename  VARCHAR(255) NULL,
    attachment_filepath  VARCHAR(500) NULL,
    attachment_filesize  VARCHAR(20)  NULL,
    created_at           TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- Employees: Engineering / Sarah Adams (manager_id=4)
INSERT INTO users (id,employee_id,name,email,username,password,role,department_id,department_name,manager_id,skills,workload,hours_per_week,performance_score,status,avatar_initials,join_date) VALUES
...
-- Employees: Engineering / Chris Morgan (manager_id=5)
INSERT INTO users (id,employee_id,name,email,username,password,role,department_id,department_name,manager_id,skills,workload,hours_per_week,performance_score,status,avatar_initials,join_date) VALUES
...
-- Employees: DevOps / Robert Brown (manager_id=6)
INSERT INTO users (id,employee_id,name,email,username,password,role,department_id,department_name,manager_id,skills,workload,hours_per_week,performance_score,status,avatar_initials,join_date) VALUES
...
-- Employees: DevOps / Jessica Williams (manager_id=7)
INSERT INTO users (id,employee_id,name,email,username,password,role,department_id,department_name,manager_id,skills,workload,hours_per_week,performance_score,status,avatar_initials,join_date) VALUES
...
(4,'Sarah Adams',    'comment','Prioritize the checkout flow — client review on the 15th', 1,  '2026-03-06'),
...
-- ============================================================
-- VERIFY (run these manually after import to confirm)
-- ============================================================
...
-- ============================================================
-- MIGRATION SCRIPTS — Run these if upgrading an EXISTING DB
-- (Hibernate ddl-auto=update handles this automatically,
--  but run manually if needed in DBeaver)
-- ============================================================

-- Bug Fix #5: Add attachment columns to existing messages table
ALTER TABLE messages
    ADD COLUMN IF NOT EXISTS attachment_filename VARCHAR(255) NULL,
    ADD COLUMN IF NOT EXISTS attachment_filepath VARCHAR(500) NULL,
    ADD COLUMN IF NOT EXISTS attachment_filesize VARCHAR(20)  NULL;

-- Bug Fix #8: Add company_id to existing users table
ALTER TABLE users
